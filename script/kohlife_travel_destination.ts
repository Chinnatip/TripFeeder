...
fs.readFile('credentials.json', (err, content) => {
  ...
  const parseContent: any = content
  DOCUMENT_LISTS.map(doc => {
    authorize(JSON.parse(parseContent), printDocTitle, doc)
  })
})

const authorize = (credentials, callback, doc) => {
    ...
    (err, res) => {
      ...
      content.map(({ paragraph }) => {
        if (paragraph !== undefined) {
          ...
          elements.map(({ textRun }) => {
            const {
              textStyle: { bold },
              content
            } = textRun
            if (content !== '\n' && content !== ' \n' && content !== ' ') {
              const trimText = content.replace('\n', '').trim()
