// MASTER FILE
// https://docs.google.com/spreadsheets/d/1nbtJyLxElDSBTdtzbodJxNuIxFpYDQahW8-CSCjkvqo/edit#gid=1898273787
//
// GoogleSpreadsheet module
// https://www.npmjs.com/package/google-spreadsheet
//
var GoogleSpreadsheet = require('google-spreadsheet');
var async = require('async');
//
var sheet;
var sheet_key = '1nbtJyLxElDSBTdtzbodJxNuIxFpYDQahW8-CSCjkvqo';
var doc = new GoogleSpreadsheet(sheet_key);
var creds_json = {
    private_key: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n',
    client_email: 'kohlife-admin@quickstart-1572411459716.iam.gserviceaccount.com'
};
// Sheet Finder
var findSheet = function (sheetName, info) {
    var sheet_index = 0;
    info.worksheets.map(function (sheet, index) {
        if (sheet.title === sheetName) {
            sheet_index = index;
        }
    });
    return info.worksheets[sheet_index];
};
async.series([
    // Authentication
    function Auth(step) {
        doc.useServiceAccountAuth(creds_json, step);
    },
    // Select working sheet
    function selectWorkingSheet(step) {
        var sheet_name = 'trip';
        doc.getInfo(function (err, info) {
            sheet = findSheet(sheet_name, info);
            step();
        });
    },
    // Read the rows
    function selectRows(step) {
        sheet.getRows({
            offset: 1,
            limit: 20,
            orderby: 'col2'
        }, function (err, rows) {
            rows.map(function (row) {
                var tripid = row.tripid, operator = row.operator, tripname = row.tripname, tripsummary = row.tripsummary, departure = row.departure, arrival = row.arrival, durationhour = row.durationhour, durationmin = row.durationmin, status = row.status, adultnetprice = row.adultnetprice, kidnetprice = row.kidnetprice;
                console.log(tripid + " | " + operator + " | " + tripname);
            });
            step();
        });
    }
    // Add new row
    // function managingSheets(step) {
    //   sheet.addRow(
    //     {
    //       booking_pnr: '34UI9P',
    //       from: 'Koh phagan',
    //       to: 'Suratthani',
    //       date: '6-Nov-2019',
    //       amount: 720,
    //       contact_name: 'Tryn Ninlakorn',
    //       nationality: 'TH',
    //       email: 'tryn@kohlife.com',
    //       telephone_no: '090-988-9696',
    //       dropoff: 'Ramada Inn, lamai'
    //     },
    //     function() {
    //       console.log('added')
    //     }
    //   )
    //   step()
    // }
], function (err) {
    if (err) {
        console.log('Error: ' + err);
    }
});
//# sourceMappingURL=kohcloud_sheet_read.js.map